"""Native extraction primitives used by :mod:`app.services.extractor`."""

from ._native import (
    NativeDocumentBlocks,
    NativeDocumentIRV2,
    NativeExtraction,
    NativeIRElementV2,
    NativeIRListItemV2,
    NativeIRListV2,
    NativeIRMathV2,
    NativeIRSerializationV2,
    NativeIRTableCellV2,
    NativeIRTableV2,
    NativeIRTextRunV2,
    NativeLocalAtomicBatchItemV0,
    NativeOrderedSourceTextMapV2,
    NativeOrderedSourceTextSpanV2,
    NativeSelectionCertificateV0,
    NativeSelectionReceiptV0,
    NativeSelectionReplayV0,
    NativeSemanticBlock,
    backend_version,
    extract_document_blocks,
    extract_document_ir_v2_native,
    extract_html,
    packaged_source_digest,
)
from .document_ir import (
    DEFAULT_DOCUMENT_IR_LIMITS,
    DocumentIRLimits,
    extract_document_ir,
)
from .document_ir_v2 import (
    DEFAULT_DOCUMENT_IR_V2_LIMITS,
    DocumentIRV2Limits,
    extract_document_ir_v2,
    reconstruct_document_ir_v2,
)
from .local_atomic_batch_v0 import (
    LOCAL_ATOMIC_SELECTION_BATCH_V0_CONTRACT,
    LOCAL_ATOMIC_SELECTION_BATCH_V0_MAX_ATOMS,
    LocalAtomicBatchItemV0,
    create_local_atomic_selection_batch_v0,
    verify_and_replay_local_atomic_selection_batch_v0,
)
from .selection_atom_catalog_v1 import (
    DEFAULT_SELECTION_ATOM_CATALOG_V1_CONFIG,
    SELECTION_ATOM_CATALOG_V1_SCHEMA,
    SELECTION_ATOM_V1_SCHEMA,
    SelectionAtomCatalogV1,
    SelectionAtomCatalogV1Config,
    SelectionAtomKindV1,
    SelectionAtomV1,
    build_selection_atom_catalog_from_document_v1,
    build_selection_atom_catalog_v1,
)
from .selection_certificate_v0 import (
    DEFAULT_SELECTION_CERTIFICATE_V0_MAX_OUTPUT_BYTES,
    SELECTION_CERTIFICATE_V0_MAX_SELECTIONS,
    SelectionCertificateV0,
    SelectionReceiptV0,
    SelectionReplayV0,
    create_local_atomic_selection_certificate_v0,
    create_selection_certificate_v0,
    decode_selection_certificate_v0,
    verify_and_replay_local_atomic_selection_certificate_v0,
    verify_and_replay_selection_certificate_v0,
)
from .source_text_mapper_v2 import (
    DEFAULT_ORDERED_SOURCE_TEXT_MAP_V2_LIMITS,
    ORDERED_SOURCE_TEXT_MAP_V2_SCHEMA,
    ORDERED_SOURCE_TEXT_SPAN_V2_SCHEMA,
    OrderedSourceTextMapV2,
    OrderedSourceTextMapV2Limits,
    OrderedSourceTextSpanV2,
    map_ordered_source_text_v2,
)

__all__ = [
    "DEFAULT_DOCUMENT_IR_LIMITS",
    "DEFAULT_DOCUMENT_IR_V2_LIMITS",
    "DEFAULT_SELECTION_CERTIFICATE_V0_MAX_OUTPUT_BYTES",
    "DEFAULT_SELECTION_ATOM_CATALOG_V1_CONFIG",
    "DEFAULT_ORDERED_SOURCE_TEXT_MAP_V2_LIMITS",
    "ORDERED_SOURCE_TEXT_MAP_V2_SCHEMA",
    "ORDERED_SOURCE_TEXT_SPAN_V2_SCHEMA",
    "SELECTION_CERTIFICATE_V0_MAX_SELECTIONS",
    "SELECTION_ATOM_CATALOG_V1_SCHEMA",
    "SELECTION_ATOM_V1_SCHEMA",
    "DocumentIRLimits",
    "DocumentIRV2Limits",
    "NativeDocumentBlocks",
    "NativeDocumentIRV2",
    "NativeExtraction",
    "NativeIRElementV2",
    "NativeIRListItemV2",
    "NativeIRListV2",
    "NativeIRMathV2",
    "NativeIRSerializationV2",
    "NativeSemanticBlock",
    "NativeIRTableCellV2",
    "NativeIRTableV2",
    "NativeIRTextRunV2",
    "NativeLocalAtomicBatchItemV0",
    "NativeOrderedSourceTextMapV2",
    "NativeOrderedSourceTextSpanV2",
    "NativeSelectionCertificateV0",
    "NativeSelectionReceiptV0",
    "NativeSelectionReplayV0",
    "LOCAL_ATOMIC_SELECTION_BATCH_V0_CONTRACT",
    "LOCAL_ATOMIC_SELECTION_BATCH_V0_MAX_ATOMS",
    "LocalAtomicBatchItemV0",
    "OrderedSourceTextMapV2",
    "OrderedSourceTextMapV2Limits",
    "OrderedSourceTextSpanV2",
    "backend_version",
    "SelectionCertificateV0",
    "SelectionAtomCatalogV1",
    "SelectionAtomCatalogV1Config",
    "SelectionAtomKindV1",
    "SelectionAtomV1",
    "SelectionReceiptV0",
    "SelectionReplayV0",
    "create_local_atomic_selection_certificate_v0",
    "create_local_atomic_selection_batch_v0",
    "create_selection_certificate_v0",
    "decode_selection_certificate_v0",
    "extract_document_blocks",
    "extract_document_ir",
    "extract_document_ir_v2",
    "extract_document_ir_v2_native",
    "extract_html",
    "map_ordered_source_text_v2",
    "packaged_source_digest",
    "reconstruct_document_ir_v2",
    "build_selection_atom_catalog_from_document_v1",
    "build_selection_atom_catalog_v1",
    "verify_and_replay_selection_certificate_v0",
    "verify_and_replay_local_atomic_selection_certificate_v0",
    "verify_and_replay_local_atomic_selection_batch_v0",
]
